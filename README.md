...
One community can feed two rows: **VK** (members + wall post likes/views) and **VK Video** (video likes/views — a VK Video channel is its backing community). The video row deliberately shows a dash for followers so the community's members aren't counted twice in the tray total.
...
2. Copy the **сервисный ключ доступа** from the app's settings into `providers.vk.service_token` (and `providers.vkvideo.service_token` — the same key works for both).
3. Set `group` to the community's screen name or numeric id (find the id in any video URL: `vkvideo.ru/video-XXXXXXX_YYY`).

Note: the docs claim `video.get` needs a user token — in practice the service key works. A community access key from group settings is *not* enough for either walk.
...
| **VK Video** | shared with VK — one community, one number (the popup merges the cell) | likes summed over every video in the community's Video section | plays summed over those videos³ |
...
² VK's post counter is *feed impressions*, not video plays — the two VK rows measure different things and never double-count.
³ **VK clips are not counted**: the public API exposes them neither as a list nor as an aggregate (the `clips_views` counter exists in VK's schema but comes back empty for external apps). Long-form videos are counted in full; the clips gap is visible only next to the creator cabinet's own analytics.
...
| `service_token` | vk, vkvideo | — | Service key of any VK ID app; one key can serve both rows |
| `group` | vk, vkvideo | — | Community screen name or numeric id (no minus) |
| `count_views` | tiktok, instagram, telegram, vk, vkvideo | `true` | Off = skip the views calls |
| `views_refresh_min` | instagram, telegram, vk, vkvideo | `15` | Minutes between views/likes passes |
...
- **VK / VK Video** walk 100 items per request under the service key's 5 rps; both walks are cached for `views_refresh_min` minutes, so a poll normally costs one `groups.getById`. The historical (now undocumented) `wall.get` quota of ~5000 calls/day only matters past several thousand posts.
...
Одно сообщество кормит две строки: **VK** (подписчики + лайки/показы постов стены) и **VK Video** (лайки/просмотры роликов — канал ВК Видео и есть его сообщество). В видео-строке подписчики нарочно прочерком, чтобы участники сообщества не считались в сумме трея дважды.
...
2. Скопируйте **сервисный ключ доступа** из настроек приложения в `providers.vk.service_token` (и в `providers.vkvideo.service_token` — ключ один на обе строки).
3. В `group` впишите короткое имя сообщества или числовой id (id виден в ссылке любого ролика: `vkvideo.ru/video-XXXXXXX_YYY`).

Примечание: документация утверждает, что `video.get` требует пользовательский токен — на практике сервисный ключ работает. Ключа доступа сообщества из настроек группы для обходов **не** хватает.
...
| **VK Video** | общие с VK — сообщество одно, число одно (в попапе ячейка объединена) | сумма лайков роликов раздела «Видео» | проигрывания этих роликов³ |
...
² Счётчик постов VK — это *показы в ленте*, а не проигрывания видео: две VK-строки меряют разное и никогда не задваиваются.
³ **Клипы VK не считаются**: публичный API не отдаёт их ни списком, ни агрегатом (поле `clips_views` в схеме VK существует, но внешним приложениям приходит пустым). Длинные ролики считаются полностью; разрыв по клипам виден только рядом с собственной аналитикой кабинета автора.
...
| `service_token` | vk, vkvideo | — | Сервисный ключ любого приложения VK ID; один ключ на обе строки |
| `group` | vk, vkvideo | — | Короткое имя сообщества или числовой id (без минуса) |
| `count_views` | tiktok, instagram, telegram, vk, vkvideo | `true` | Выкл = не запрашивать просмотры |
| `views_refresh_min` | instagram, telegram, vk, vkvideo | `15` | Минуты между проходами за просмотрами/лайками |
...
- **VK / ВК Видео** обходятся по 100 элементов за запрос при лимите сервисного ключа 5 запросов/сек; оба обхода кэшируются на `views_refresh_min` минут, так что обычный опрос стоит один `groups.getById`. Историческая (ныне недокументированная) квота `wall.get` ~5000 вызовов/сутки начинает мешать только после нескольких тысяч постов.
